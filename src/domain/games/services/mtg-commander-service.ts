...
import { IGameEvent } from "../../interfaces/IGame";
...
export const SetPlayerDefaults = (newPlayer: Player, room:Room)=>{
    ...
    room.players.forEach((oldPlayer:Player)=>{
        if(oldPlayer.id != newPlayer.id){
            ...
            oldPlayer.commanders.forEach((commander:IPlayingCard)=>{
...
export const SetCommander = (gameEvent: IGameEvent, room:Room)=>{
    ...
    let oldCommander:IPlayingCard = gameEvent.payload.index < gameEvent.callingPlayer.commanders.length ?  gameEvent.callingPlayer.commanders[gameEvent.payload.index] : null;
