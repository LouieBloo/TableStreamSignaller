...
export async function search(query: string): Promise<IPlayingCard[]> {
  try {
    ...
    return response && response.data && response.data.data ? response.data.data.map((card:IPokemonCard)=>{return mapToPlayingCard(card)}) : [];
  } catch (error) {
...
export const mapToPlayingCard = (pokemonCard:IPokemonCard):IPlayingCard=> {
  return {
    id: pokemonCard.id,
